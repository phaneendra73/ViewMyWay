...
interface NotificationProps {
  message: string;
  type: 'success' | 'error' | 'unauthorized'; // Add 'unauthorized' to the type options
  onClose: () => void;
...
const Notification: React.FC<NotificationProps> = ({
  ...
  const getMessageColor = () => {
    switch (type) {
      case 'success':
        return 'text-green-600'; // Green text for success
      case 'error':
        return 'text-red-600'; // Red text for error
      case 'unauthorized':
        return 'text-orange-600'; // Orange text for unauthorized
      default:
  ...
  return (
    <div
      className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-6 rounded shadow-lg flex flex-col items-center`}
